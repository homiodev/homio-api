...
public abstract class DependencyExecutableInstaller implements UIActionHandler {
    ...
    @SneakyThrows
    public static Path downloadAndExtract(
            @NotNull String url,
    ...
    /**
     * If set - scan DependencyExecutableInstaller and listen when button fires on ui and handle
     * installation
     */
    public abstract @Nullable Class<? extends SettingPluginButton> getInstallButton();

    protected abstract @Nullable Path installDependencyInternal(
            @NotNull EntityContext entityContext, @NotNull ProgressBar progressBar)
            throws Exception;

    protected void afterDependencyInstalled(
            @NotNull EntityContext entityContext, @Nullable Path path) {}
    ...
    public synchronized boolean isRequireInstallDependencies(
            @NotNull EntityContext entityContext, boolean useCacheIfPossible) {
        if (requireInstall == null || !useCacheIfPossible) {
            requireInstall = true;
            MachineHardwareRepository repository =
    ...
    public boolean checkDependencyInstalled(
            @NotNull EntityContext entityContext, @NotNull MachineHardwareRepository repository) {
        Path targetPath =
                Paths.get(entityContext.setting().getValue(getDependencyPluginSettingClass()));
    ...
    public boolean checkWinDependencyInstalled(
            @NotNull MachineHardwareRepository repository, @NotNull Path targetPath) {
    ...
    public void runService(
            EntityContext entityContext, Consumer<Process> processConsumer, String entityID) {
    ...
    @Override
    public ActionResponseModel handleAction(
            @NotNull EntityContext entityContext, @NotNull JSONObject ignore) {
        if (isRequireInstallDependencies(entityContext, false)) {
            entityContext
                    .bgp()
                    .runWithProgress(
                            "install-deps-" + getClass().getSimpleName(),
                            false,
                            progressBar -> installDependency(entityContext, progressBar),
                            null,
