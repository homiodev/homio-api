...
public abstract class Scratch3BaseFileSystemExtensionBlocks<
                T extends BundleEntrypoint, E extends BaseEntity & BaseFileSystemEntity>
    ...
    public Scratch3BaseFileSystemExtensionBlocks(
            String color, EntityContext entityContext, T bundleEntryPoint, Class<E> entityClass) {
        ...
        this.uploadOptionsMenu =
                menuStatic("uploadOptionsMenu", UploadOption.class, UploadOption.Overwrite)
                        .setMultiSelect(" | ");
        ...
        blockCommand(
                15,
                "modify_file",
                "Update [VALUE] as [NAME] to [PARENT] of [ENTITY] | Options: [OPTIONS]",
                this::sendFileHandle,
                block -> {
        ...
        blockReporter(
                30,
                "get_count",
                "Count of [VALUE] in [PARENT] [ENTITY]",
        ...
        switch (countNodeEnum) {
            case Files:
                return new DecimalType(
                        children.stream().filter(c -> !c.getAttributes().isDir()).count());
            case Folders:
                return new DecimalType(
                        children.stream().filter(c -> c.getAttributes().isDir()).count());
            case All:
    ...
    private DecimalType getTotalQuotaReporter(WorkspaceBlock workspaceBlock) {
        ...
        return new DecimalType(
                getDrive(workspaceBlock).getFileSystem(entityContext).getTotalSpace() / unit);
    }

    private DecimalType getUsedQuotaReporter(WorkspaceBlock workspaceBlock) {
        double unit = workspaceBlock.getMenuValue("UNIT", this.unitMenu).divider;
        return new DecimalType(
                getDrive(workspaceBlock).getFileSystem(entityContext).getUsedSpace() / unit);
    }
    ...
    private void deleteFileHandle(WorkspaceBlock workspaceBlock) {
        ...
        if (!"-".equals(fileId)) {
            try {
                getDrive(workspaceBlock)
                        .getFileSystem(entityContext)
                        .delete(Collections.singleton(fileId));
            } catch (Exception ex) {
                workspaceBlock.logErrorAndThrow(
                        "Unable to delete file: <{}>. Msg: ",
                        fileId,
        ...
        if (!"-".equals(fileId)) {
            ...
            return new RawType(
                    ...
                    StringUtils.defaultIfEmpty(
                            treeNode.getAttributes().getContentType(),
    ...
    private void sendFileHandle(WorkspaceBlock workspaceBlock) {
        String fileName =
                workspaceBlock.getInputStringRequired("NAME", "Send file block requires file name");
        ...
        try {
            ...
            List<UploadOption> uploadOptions =
                    workspaceBlock.getMenuValues(
                            "OPTIONS", this.uploadOptionsMenu, UploadOption.class, "~~~");
            ...
            if (!uploadOptions.contains(UploadOption.Overwrite)) {
                ...
                uploadOption =
                        uploadOptions.contains(UploadOption.Append)
                                ? FileSystemProvider.UploadOption.Append
                                : FileSystemProvider.UploadOption.Replace;
            }

            fileSystem.copy(TreeNode.of(fileName, value), folderId, uploadOption);
        } catch (Exception ex) {
            workspaceBlock.logError(
                    "Unable to store file: <{}>. Msg: <{}>", fileName, ex.getMessage());
        }
    }
