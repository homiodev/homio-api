...
import static org.touchhome.bundle.api.entity.micro.MicroControllerBaseEntity.ScanMicroControllers;
...
public abstract class MicroControllerBaseEntity<T extends MicroControllerBaseEntity> extends DeviceBaseEntity<T> {
    ...
    public static class ScanMicroControllers implements UISidebarButtonHandler {
        ...
        @Override
        public ActionResponseModel apply(EntityContext entityContext) {
            if (started.compareAndSet(false, true)) {
                entityContext.bgp().run("scan-micro-controllers", () -> {
                    log.info("Start scan for controllers");
                    entityContext.ui().showAlwaysOnViewNotification("MC.SCAN_INFO", null, "fas fa-hourglass-end fa-spin", "#5A3E18", null, null);
                    try {
                ...
                return ActionResponseModel.showWarn("MC.SCAN_CONTROLLER_ALREADY_STARTED");
            }
            return ActionResponseModel.showSuccess("MC.SCAN_CONTROLLERS_STARTED");
        }
    }
}
