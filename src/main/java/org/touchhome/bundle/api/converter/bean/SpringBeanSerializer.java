...
public class SpringBeanSerializer extends JsonSerializer<Object> {
    ...
    public void serialize(Object value, JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
        Map<String, ?> beans =
                ApplicationContextHolder.getBean(EntityContext.class)
                        .getBeansOfTypeWithBeanName(value.getClass());
        if (beans.isEmpty()) {
            throw new IllegalArgumentException(
                    "Unable to find bean with name: " + value.getClass());
        } else if (beans.size() > 1) {
            throw new IllegalArgumentException(
                    "Found multiple beans with name: " + value.getClass());
        }
