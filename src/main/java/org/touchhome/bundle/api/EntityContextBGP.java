...
public interface EntityContextBGP {
    ...
    default <T> ThreadContext<Map<String, T>> awaitProcesses(
            @NotNull String name,
                ...
                arg -> {
                    ...
                    for (ThreadContext<T> process : processes) {
                        try {
                            ...
                            logger.error(
                                    "Error while await <{}> for finish. Fire process termination",
                                    process.getName());
                            process.cancel();
                        }
                    }
    ...
    default ThreadContext<Void> runWithProgress(
            @NotNull String key,
            boolean cancellable,
    ...
    default <R> ThreadContext<R> runWithProgressAndGet(
            @NotNull String key,
            boolean cancellable,
                ...
                arg -> {
                    ProgressBar progressBar =
                            (progress, message) ->
                                    getEntityContext()
                                            .ui()
                                            .progress(key, progress, message, cancellable);
    ...
    /**
     * Register 'external' threads in context. 'External' mean thread that wasnt created by
     * bgp().builder(...) but eny other new Thread(...) or even java.lang.Process
     */
    void registerThreadsPuller(
            @NotNull String entityID, @NotNull Consumer<ThreadPuller> threadPullerConsumer);
    ...
    interface ThreadContext<T> {
        ...
        T await(@NotNull Duration timeout)
         ...
         * @param valueListener - listener: First 2 parameters is value from run/schedule and
         *     previous value. Return boolean where is remove listener or not after execute
