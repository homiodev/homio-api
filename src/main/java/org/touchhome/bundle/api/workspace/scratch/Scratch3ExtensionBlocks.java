...
public abstract class Scratch3ExtensionBlocks {
    ...
    public Scratch3ExtensionBlocks(
            String color, EntityContext entityContext, BundleEntrypoint bundleEntryPoint) {
    ...
    public Scratch3ExtensionBlocks(
            ...
            String idSuffix) {
        this.id =
                bundleEntryPoint == null
                        ? idSuffix
                        : bundleEntryPoint.getBundleId() + (idSuffix == null ? "" : "-" + idSuffix);
        ...
        if (color != null) {
            ...
            this.blockIconURI =
                    "data:image/png;base64,"
                            + Base64.getEncoder()
                                    .encodeToString(
                                            IOUtils.toByteArray(Objects.requireNonNull(resource)));
    ...
    protected Scratch3Block blockHat(
            int order,
            String opcode,
            String text,
        ...
        return addBlock(
                new Scratch3Block(order, opcode, BlockType.hat, text, handler, null),
                configureHandler);
    }

    protected Scratch3Block blockHat(
            int order, String opcode, String text, Scratch3Block.Scratch3BlockHandler handler) {
    ...
    protected Scratch3ConditionalBlock blockCondition(
            int order,
            String opcode,
            String text,
            Scratch3Block.Scratch3BlockHandler handler,
        ...
        return addBlock(
                new Scratch3ConditionalBlock(
                        order, opcode, BlockType.conditional, text, handler, null),
                configureHandler);
    }
    ...
    protected Scratch3Block blockCommand(
            int order,
            String opcode,
            String text,
            Scratch3Block.Scratch3BlockHandler handler,
            Consumer<Scratch3Block> configureHandler) {
        return addBlock(
                new Scratch3Block(order, opcode, BlockType.command, text, handler, null),
                configureHandler);
    }
    ...
    protected Scratch3Block blockCommand(
            String opcode, Scratch3Block.Scratch3BlockHandler handler) {
        return blockCommand(0, opcode, null, handler, null);
    }

    protected Scratch3Block blockReporter(
            int order,
            String opcode,
            String text,
    ...
    protected Scratch3Block blockReporter(
            String opcode, Scratch3Block.Scratch3BlockEvaluateHandler evalHandler) {
    ...
    */
    ...
    @SneakyThrows
    protected <T extends Scratch3Block> T blockTargetReporter(
            int order,
            String opcode,
            String text,
            Scratch3Block.Scratch3BlockEvaluateHandler evalHandler,
    ...
    protected Scratch3Block blockBoolean(
            int order,
            String opcode,
            String text,
            Scratch3Block.Scratch3BlockEvaluateHandler evalHandler,
    ...
    protected <T extends Enum> MenuBlock.StaticMenuBlock<T> menuStatic(
            String name, Class<T> enumClass, T defaultValue, Predicate<T> filter) {
        return addMenu(
                new MenuBlock.StaticMenuBlock(name, null, enumClass)
    ...
    protected MenuBlock.ServerMenuBlock menuServer(
            String name, String url, String firstKey, String firstValue, Integer... clusters) {
        return addMenu(
                new MenuBlock.ServerMenuBlock(name, url, firstKey, firstValue, clusters, true));
    }
    ...
    protected MenuBlock.ServerMenuBlock menuServer(
            String name, String url, String firstKey, String firstValue) {
        return addMenu(new MenuBlock.ServerMenuBlock(name, url, firstKey, firstValue, null, true));
    }

    protected MenuBlock.ServerMenuBlock menuServerServiceItems(
            String name, Class<?> entityServiceClass, String firstKey) {
        return addMenu(
    ...
    protected MenuBlock.ServerMenuBlock menuServerItems(
            String name, Class<? extends BaseEntity> itemClass, String firstKey) {
        return menuServerItems(name, itemClass, firstKey, "-");
    }

    protected MenuBlock.ServerMenuBlock menuServerItems(
            String name,
            Class<? extends BaseEntity> itemClass,
            String firstKey,
            String firstValue) {
        return addMenu(
                new MenuBlock.ServerMenuBlock(
                        name,
                        "rest/item/type/" + itemClass.getSimpleName(),
                        firstKey,
                        firstValue,
                        null,
                        true));
    ...
    protected MenuBlock.StaticMenuBlock<String> menuStaticList(
            String name, Map<String, String> items, String defaultValue) {
        return addMenu(
                new MenuBlock.StaticMenuBlock(name, items, String.class)
                        .setDefaultValue(defaultValue));
    }

    protected MenuBlock.ServerMenuBlock menuServerFiles(
            @NotNull MenuBlock.ServerMenuBlock fileSystemDependency, @Nullable String regexp) {
        return menuServer("FILE", defaultString(regexp, ".*"), "File")
                .setDependency(fileSystemDependency)
                .setUIDelimiter("/");
    }

    protected MenuBlock.ServerMenuBlock menuServerFolders(
            @NotNull MenuBlock.ServerMenuBlock fileSystemDependency, @Nullable String regexp) {
        return menuServer("FOLDER", defaultString(regexp, ".*"), "Folder")
                .setDependency(fileSystemDependency);
    }
    ...
    private <T extends MenuBlock> T addMenu(T menuBlock) {
        if (this.menus.put(menuBlock.getName(), menuBlock) != null) {
            throw new RuntimeException(
                    "Found multiple menu with same name: " + menuBlock.getName());
        }
        return menuBlock;
    }
}
