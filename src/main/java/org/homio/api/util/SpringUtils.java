...
public class SpringUtils {
  ...
  public static String replaceEnvValues(String text, ReplaceEnvHandler replaceEnvHandler) {
    return replaceValues(ENV_PATTERN, text, replaceEnvHandler);
  }

  public static String replaceHashValues(String text, ReplaceEnvHandler replaceEnvHandler) {
    return replaceValues(HASH_PATTERN, text, replaceEnvHandler);
  }

  @SneakyThrows
  public static String replaceValues(
      Pattern pattern, String text, ReplaceEnvHandler replaceEnvHandler) {
    Matcher matcher = pattern.matcher(text);
    StringBuffer noteBuffer = new StringBuffer();
    while (matcher.find()) {
      String group = matcher.group();
      String[] array = getSpringValuesPattern(group);
      String replace = replaceEnvHandler.replace(array[0], array[1], noteBuffer);
      matcher.appendReplacement(noteBuffer, replace);
    }
  ...
  public static String getEnvProperty(
      String value, ThrowingBiFunction<String, String, String, Exception> propertyGetter) {
  ...
  public static String replaceValues(
      String text, BiFunction<String, String, String> propertyGetter) {
    return replaceValues(
        text, propertyGetter, PATTERN, PATTERN_PREFIX_LENGTH, PATTERN_SUFFIX_LENGTH);
  }
  ...
  public static String replaceValues(
    ...
    while (matcher.find()) {
      ...
      String value =
          replaceValues(envProperty, propertyGetter, pattern, prefixLength, suffixLength);
  ...
  public static String[] getSpringValuesPattern(String value, int prefixLength, int suffixLength) {
    ...
    return valuePattern.contains(":")
        ? valuePattern.split(":", 2)
        : new String[] {valuePattern, ""};
  }
