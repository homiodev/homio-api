...
public final class CommonUtils {
  ...
  public static GitHubProject STATIC_FILES;
  ...
  static {
    // must be initialized after installPath
  ...
  public static @NotNull List<String> readFile(@NotNull String fileName) {
    try {
      try (InputStream resource = CommonUtils.getClassLoader().getResourceAsStream(fileName)) {
  ...
  public static @Nullable URL getResource(@Nullable String addonID, @NotNull String resource) {
    ...
    ArrayList<URL> urls = Collections.list(CommonUtils.getClassLoader().getResources(resource));
    if (urls.size() == 1) {
  ...
  private static List<Map<String, String>> readProperties(String path) {
    Enumeration<URL> resources = CommonUtils.getClassLoader().getResources(path);
  ...
  public static @Nullable Route getClassRoute(Class<?> clazz) {
    for (Annotation classSpecificAnnotationInstance : clazz.getDeclaredAnnotations()) {
      ...
      UIRouteMenu menuConfig =
          UIRouteMenu.class.isAssignableFrom(specificAnnotationType)
              ? clazz.getDeclaredAnnotation(UIRouteMenu.class)
              : specificAnnotationType.getAnnotation(UIRouteMenu.class);
      if (menuConfig != null) {
