...
public interface ItemDiscoverySupport {
    ...
    @Nullable
    DeviceScannerResult scan(@NotNull Context context,
                             @NotNull ProgressBar progressBar);

    default void handleDevice(String key, String name, Context context,
                              Consumer<List<String>> messageConsumer, Runnable saveHandler) {
        List<String> messages = new ArrayList<>();
        messageConsumer.accept(messages);
        context.ui().dialog().sendConfirmation("confirm-device-" + key,
                Lang.getServerMessage("NEW_DEVICE.TITLE", name),
                saveHandler, messages, "scan-devices");
    }
