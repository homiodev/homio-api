...
public class WebAddress {
  ...
  public static class WebAddressSerializer extends JsonSerializer<WebAddress> {
    ...
    @Override
    public void serialize(WebAddress web, JsonGenerator gen, SerializerProvider serializers)
        throws IOException {
      String value =
          web.icon == null
              ? ""
              : "<i class=\"%s\" style=\"color: %s\"></i>"
                  .formatted(web.icon.getIcon(), web.icon.getColor());
      ...
      value +=
          "<a target=\"_blank\" style=\"margin-left: 5px\" href=\"%s\">%s</a>"
              .formatted(address, title);
