...
public class UpdatableValue<T> {
  ...
  // when we create UpdatableValue which base on another UpdatableValue we must reflect base
  // changes.
   ...
   */
  public static <T> @NotNull UpdatableValue<T> deferred(
  ...
  public static <T> @NotNull UpdatableValue<T> wrap(
      @NotNull HasJsonData jsonData, @NotNull T defaultValue, @NotNull String name) {
    UpdatableValue<T> updatableValue = new UpdatableValue<>(name, defaultValue.getClass());
    String value = StringUtils.defaultIfEmpty(jsonData.getJsonData(name), defaultValue.toString());
    updatableValue.value = updatableValue.parse(value);
    updatableValue.lastRefreshTime = System.currentTimeMillis();
    updatableValue.addListener(s -> jsonData.setJsonData(name, s));
    return updatableValue;
  }

  public static <T> @NotNull UpdatableValue<T> ofNullable(
      @Nullable T value, @NotNull String name, @NotNull Class<T> valueType) {
    if (value == null) {
  ...
  private static @NotNull String getNameFromAnnotation(
      @Nullable Value value, @NotNull Supplier<String> defaultValueSupplier) {
    /*if (field != null) {
        return field.name();
    } else*/
    if (value != null) {
      ...
      return v.substring(
          v.indexOf(SystemPropertyUtils.PLACEHOLDER_PREFIX)
              + SystemPropertyUtils.PLACEHOLDER_PREFIX.length(),
          v.contains(SystemPropertyUtils.VALUE_SEPARATOR)
              ? v.indexOf(SystemPropertyUtils.VALUE_SEPARATOR)
  ...
  public T getFreshValue(Duration duration, Runnable updateHandler) {
    if (!fetchFreshValueStarted
        && System.currentTimeMillis() - lastRefreshTime > duration.toMillis()) {
  ...
  public T update(@NotNull T updatedValue) {
    ...
    if (value == null || !value.equals(updatedValue)) {
      ...
      // reflect Listeners must according to code order(i.e. setCorePoolSize must precede
      // setMaxPoolSize)
  ...
  public static final class UpdatableValueConverter implements ConditionalGenericConverter {
      ...
      try {
        ...
        if (!UpdatableValue.class.isAssignableFrom(targetType.getType())) {
          throw new RuntimeException(
              "Failed cast targetType <"
                  + targetType.getType()
                  + "> to UpdatableValue in "
                  + targetType.getSource());
        }
        String name =
            getNameFromAnnotation(
                /*targetType.getAnnotation(Column.class),*/
                targetType.getAnnotation(Value.class),
                () -> {
                  // must never through as UpdatableValueConverter uses by spring only with @Value
                  // annotation
                  throw new RuntimeException(
                      "Can not fetch UpdatableValue name from " + targetType.getSource());
                });
        UpdatableValue updatableValue = new UpdatableValue<>(name, genericClass);

        updatableValue.validators =
            collectValidators(
                targetType,
                a -> {
    ...
    private @NotNull Set<Validator> collectValidators(
        @NotNull TypeDescriptor targetType, @NotNull Function<Annotation, Long> exceedConverter) {
              ...
              a ->
                  (Validator)
                      value -> {
                        ...
                        if ((actualValue < exceedValue
                                && a.annotationType().isAssignableFrom(Min.class))
                            || (actualValue > exceedValue
                                && a.annotationType().isAssignableFrom(Max.class))) {
                          throw new IllegalArgumentException(
                              String.format(
                                  "Validation fails for value <%s>. %s value is <%d>",
                                  value, a.annotationType().getSimpleName(), exceedValue));
                        }
                      })
