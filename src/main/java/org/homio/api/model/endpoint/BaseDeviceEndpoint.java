...
public abstract class BaseDeviceEndpoint<D extends DeviceEndpointsBehaviourContract>
    implements DeviceEndpoint {

  @NotNull private final Map<String, Consumer<State>> changeListeners = new ConcurrentHashMap<>();

  @NotNull @Getter private final String group;
  ...
  @Getter @Setter private String endpointEntityID;

  @Getter private D device;

  @Getter @Setter @Nullable private String unit;

  @Getter private long updated = System.currentTimeMillis();

  @Getter @NotNull private State value = new StringType("N/A");

  @Nullable private Object dbValue;

  @Nullable @Getter private String variableID;

  @Getter @Setter private boolean readable = true;

  @Getter @Setter private boolean writable = false;

  @Getter @Setter private String endpointName;

  @Getter @Setter private EndpointType endpointType;

  @Getter @Setter private int order = -1;

  @Nullable private ConfigDeviceDefinitionService configService;

  @Getter @Setter @Nullable private Float min;

  @Getter @Setter @Nullable private Float max;

  @Getter @Setter @Nullable private List<OptionModel> range;

  @Getter @Setter @Nullable private Object defaultValue;

  @JsonIgnore @Nullable private Set<String> alternateEndpoints;

  @Setter @Nullable private ConfigDeviceEndpoint configEndpoint;

  @Getter @Setter private boolean visibleEndpoint = true;

  @Getter @Setter @Nullable private Supplier<Boolean> visibleEndpointHandler;

  @Setter private boolean ignoreDuplicates = true;

  @Setter @Getter private boolean stateless;

  @Nullable private ThrowingConsumer<State, Exception> updateHandler;

  @Setter private boolean dbValueStorable;
  ...
  public void init(
    ...
    configEndpoint =
        endpointEntityID == null ? null : configService.getDeviceEndpoints().get(endpointEntityID);
    ...
    if (endpointEntityID == null) {
      throw new IllegalStateException(
          "Unable to create device endpoint without endpoint id. " + endpointName);
    }
      ...
      case ENDPOINT_DEVICE_STATUS:
        ...
        if (range == null) {
          if (configEndpoint != null && configEndpoint.getAvailableValues() != null) {
            range =
                OptionModel.enumList(
                    Status.class,
                    status -> configEndpoint.getAvailableValues().contains(status.name()));
          } else {
    ...
    if (configEndpoint != null) {
      ...
      if (endpointType == EndpointType.select
          && range == null
          && configEndpoint.getAvailableValues() != null) {
  ...
  public void writeValue(@NotNull State state) {
    ...
    switch (getEndpointType()) {
      ...
      case number, dimmer -> {
        targetState =
            state instanceof DecimalType ? state : new DecimalType(state.intValue(), scale);
  ...
  /**
   * Set list of alternative endpoint names if endpointEntityID is null or if
   * configService.getDeviceEndpoints().get(endpointEntityID) == null
   *
  ...
  public final @NotNull String getDeviceID() {
    return requireNonNull(
        StringUtils.defaultIfEmpty(device.getIeeeAddress(), device.getEntityID()));
  }
  ...
  public @Nullable String getOrCreateVariable() {
    ...
    if (variableID == null) {
      ...
      Consumer<VariableMetaBuilder> variableMetaBuilder =
          builder -> {
            builder
                .setVarDescription(getVarDescription())
                .setOwner(device)
                .setIcon(icon)
                .setLocked(true);
      ...
      if (variableType == VariableType.Enum) {
        Set<String> range =
            getVariableEnumValues().stream().map(OptionModel::getKey).collect(Collectors.toSet());
        variableID =
            context
                .var()
                .createEnumVariable(
  ...
  private boolean isEndpointExcluded() {
    return device instanceof HasExcludeEndpoints hee
        && hee.getExcludeEndpoints().contains(getName(true));
  }
  ...
  public void updateUI() {
    context
        .ui()
        .updateInnerSetItem(
            device, "endpoints", endpointEntityID, getEntityID(), new DeviceEndpointUI(this));
  }

  public String getVariableGroupID() {
    throw new IllegalStateException(
        "Variable group id must be implemented in sub class if create variable");
  }

  public ContextVar.VarDescription getVarDescription() {
    throw new IllegalStateException(
        "Variable description has to be implemented in sub class if create variable");
  }
  ...
  protected @Nullable Consumer<VariableMetaBuilder> getVariableMetaBuilder() {
    return builder -> {
      ...
      if (range != null && !range.isEmpty()) {
        attributes.add(
            "range:"
                + range.stream().map(OptionModel::getTitleOrKey).collect(Collectors.joining(";")));
      }
  ...
  public void setDevice(D device) {
    ...
    if (isEndpointExcluded()) {
      ...
      context.var().removeVariable(buildVariableID(), this);
          ...
          we can't execute this, because setDevice fires during device loading
          getOrCreateVariable();
      }*/
  }
}
