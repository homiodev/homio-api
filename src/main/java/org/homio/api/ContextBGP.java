...
public interface ContextBGP {
  ...
  void ping(
      ...
      @NotNull Consumer<Boolean> availableStatus);
  ...
  default @NotNull ProcessBuilder processBuilder(
      @NotNull DeviceBaseEntity entity, @NotNull Logger log) {
    return processBuilder(entity.getEntityID())
        ...
        .setErrorLoggerOutput(
            msg -> log.error("[{}]: {}: {}", entity.getEntityID(), entity.getTitle(), msg))
        .setInputLoggerOutput(
            msg -> log.info("[{}]: {}: {}", entity.getEntityID(), entity.getTitle(), msg));
  }
   ...
   * @return this
   */
  default @NotNull <T> ThreadContext<Map<String, T>> awaitProcesses(
      @NotNull String name,
        ...
        arg -> {
          ...
          for (ThreadContext<T> process : processes) {
            try {
              ...
              logger.error(
                  "Error while await <{}> for finish. Fire process termination", process.getName());
              process.cancel();
            }
          }
  ...
  /**
   * Run file watchdog. Check file's lastModification updates every 10 seconds and call
   * onUpdateCommand if file had been changed
   *
   ...
   */
  @NotNull
  ThreadContext<Void> runFileWatchdog(
      @NotNull Path file, String key, @NotNull ThrowingRunnable<Exception> onUpdateCommand)
   ...
   * @param eventsToListen - default is: StandardWatchEventKinds.ENTRY_CREATE,
   *     StandardWatchEventKinds.ENTRY_DELETE, StandardWatchEventKinds.ENTRY_MODIFY
   * @return - thread context
   * @throws IllegalArgumentException if directory not exists or dir already under watching
   */
  @NotNull
  ThreadContext<Void> runDirectoryWatchdog(
      @NotNull Path dir,
      @NotNull ThrowingConsumer<WatchEvent<Path>, Exception> onUpdateCommand,
      Kind<?>... eventsToListen);
  ...
  /**
   * Register 'external' threads in context. 'External' mean thread that wasn't created by
   * bgp().builder(...) but eny other new Thread(...) or even java.lang.Process
   *
   * @param id - distinguish id
   * @param threadPullerConsumer - thread object consumer
   */
  void registerThreadsPuller(
      @NotNull String id, @NotNull Consumer<ThreadPuller> threadPullerConsumer);

  <P extends HasEntityIdentifier, T> void runInBatch(
  ...
  interface ProgressBuilder {
    ...
    @SneakyThrows
    default CompletableFuture<Void> executeSync(
  ...
  interface ProcessBuilder {
    ...
    @NotNull
    ProcessBuilder onFinished(
        @NotNull
            ThrowingBiConsumer<@Nullable Exception, @NotNull Integer, Exception> finishHandler);
  ...
  interface ScheduleBuilder<T> {
    ...
    @NotNull
    default ThreadContext<T> execute(
    ...
    @NotNull
    ThreadContext<T> execute(
        @NotNull ThrowingFunction<ThreadContext<T>, T, Exception> command, boolean start);
     ...
     * @param valueListener - listener: First 2 parameters is value from run/schedule and previous
     *     value. Return boolean where is remove listener or not after execute
     * @return this
     */
    @NotNull
    ScheduleBuilder<T> valueListener(
        @NotNull String name, @NotNull ThrowingBiFunction<T, T, Boolean, Exception> valueListener);
  }

  interface ThreadPuller {

    @NotNull
    ThreadPuller addThread(
        @NotNull String name,
        @Nullable String description,
        @NotNull Date creationTime,
        @Nullable String state,
        @Nullable String errorMessage,
        @Nullable String bigDescription);

    @NotNull
    ThreadPuller addScheduler(
        @NotNull String name,
        @Nullable String description,
        @NotNull Date creationTime,
        @Nullable String state,
  ...
  interface ThreadContext<T> {
    ...
    /**
     * Reset thread. Set delay to initial state. If thread already finished - start it again. Start
     * if not started yet
     */
    void reset();
     ...
     * @param valueListener - listener handler. Remove listener from next calls if value handler
     *     return true
     * @return if listener was added successfully or false if listener with same name already exists
     */
    boolean addValueListener(
        @NotNull String name, @NotNull ThrowingBiFunction<T, T, Boolean, Exception> valueListener);
