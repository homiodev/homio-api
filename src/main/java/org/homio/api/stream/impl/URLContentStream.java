...
public class URLContentStream extends UrlResource implements ContentStream {
  ...
  @Nullable private Socket shoutCastSocket;
  ...
  public URLContentStream(@NotNull URL url, @Nullable StreamFormat streamFormat) {
    super(url);
    this.streamFormat =
        requireNonNullElseGet(streamFormat, () -> StreamFormat.evaluateFormat(getFilename()));
  }
  ...
  private @NotNull InputStream getM3U8InputStream() {
    if (content == null) {
      try (BufferedReader reader =
          new BufferedReader(
              new InputStreamReader(getURL().openStream(), StandardCharsets.UTF_8))) {
        this.content =
            reader
                .lines()
                .map(
                    line -> {
                      if (!line.isEmpty() && !line.startsWith("#")) {
                        ...
                        contentStream =
                            new URLContentStream(createRelativeUrl(line)) {
                              @Override
                              protected @NotNull String createStreamUrl(
                                  @NotNull ContentStream contentStream) {
