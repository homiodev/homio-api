...
public interface HasCustomWidget extends
  ...
  default @NotNull BaseEntity createWidget(@NotNull Context context, @NotNull String name, @NotNull String tabId, int width, int height) {
    return context.widget().createCustomWidget(getEntityID(), tabId,
  ...
  interface CallServiceMethod {
    @Nullable JsonNode callService(@NotNull Context context, @NotNull JsonNode params);
  }
}