...
public class ComposeTemplateWidget implements TemplateWidgetBuilder {
  ...
  public static final String[] LEFT_ENDPOINTS =
      new String[] {ENDPOINT_BATTERY, "power", "consumption", "energy", "voltage"};
  ...
  public void buildWidget(WidgetRequest widgetRequest) {
    ...
    if (composeContainer == null || composeContainer.isEmpty()) {
      throw new IllegalArgumentException(
          "Unable to create compose widget without compose endpoints");
    }
    ...
    for (int i = 0; i < composeContainer.size(); i++) {
      ...
      int innerWidgetHeight =
          Math.round(wdMinRowHeight / (float) sumOfRowHeights * sumOfRowHeightsAdjusted);
  ...
  private List<Integer> calcLayoutRows(
      WidgetRequest widgetRequest, List<WidgetDefinition> compose) {
