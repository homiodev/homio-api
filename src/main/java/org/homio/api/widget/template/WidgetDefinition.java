...
public class WidgetDefinition {
  ...
  @Getter private @NotNull WidgetType type;
  @Getter private @Nullable ColorPicker background;
  @Getter private boolean autoDiscovery;
  @Getter private @Nullable String leftEndpoint; // "none" special key to ignore any left endpoint
  @Getter private @Nullable String centerEndpoint;
  @Getter private @Nullable String rightEndpoint;
  ...
  @Getter private @Nullable String layout;
  @Getter private Options options = new Options();
  @Getter private @Nullable List<Requests> requests;
  @Getter private Margin margin;
    ...
    if (this.isAutoDiscovery()) {
      if (type == WidgetType.toggle) {
        return entity.getDeviceEndpoints().values().stream()
            .filter(
                p ->
                    AUTO_DISCOVERY_REGEXP.matcher(p.getEndpointName()).matches()
  ...
  public List<DeviceEndpoint> getIncludeEndpoints(MainWidgetRequest request) {
    Set<String> topIncludeEndpoints =
        request.getWidgetRequest().includeEndpoints().stream()
            .map(DeviceEndpoint::getEndpointEntityID)
            .collect(Collectors.toSet());
    List<DeviceEndpoint> allPossibleEndpoints =
        request.getItem().getEndpoints(request.getWidgetRequest().entity());
  ...
  public @Nullable WidgetDefinition.ItemDefinition getEndpoint(String key) {
    return props == null
        ? null
        : props.stream().filter(f -> f.name.equals(key)).findAny().orElse(null);
  }
