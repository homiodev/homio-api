...
public interface TemplateWidgetBuilder {
  ...
  Map<WidgetType, TemplateWidgetBuilder> WIDGETS =
      Map.of(
  ...
  Map<String, IconEndpointBuilder> ICON_ANIMATE_ENDPOINTS =
      Map.of(
    ...
    if (endpoint != null) {
      if (ENDPOINT_LAST_SEEN.equals(endpoint.getEndpointEntityID())) {
        createSimpleEndpoint(
            ...
            endpoint,
            builder -> {
              builder.setValueConverter(
                  "return Math.floor((new Date().getTime() - value) / 60000);");
  ...
  static <T extends HasSingleValueDataSource<?> & HasSetSingleValueDataSource<?>>
      T setValueDataSource(T builder, Context context, DeviceEndpoint endpoint) {
  ...
  static void buildIconAndColor(
    ...
    if (icon != null) {
      iconBuilder.setIcon(
          Objects.toString(icon.getValue(), endpoint.getIcon().getIcon()),
          (Consumer<ThresholdBuilder>)
              iconThresholdBuilder ->
                  TemplateWidgetBuilder.buildThreshold(
                      widgetRequest, icon.getThresholds(), iconThresholdBuilder));
      ColorPicker color = itemDefinition.getIconColor();
      if (color == null || (isEmpty(color.getValue()) && color.getThresholds() == null)) {
        return;
      }
      iconBuilder.setIconColor(
          Objects.toString(color.getValue(), endpoint.getIcon().getColor()),
          (Consumer<ThresholdBuilder>)
              thresholdBuilder ->
                  TemplateWidgetBuilder.buildThreshold(
                      widgetRequest, color.getThresholds(), thresholdBuilder));
    }
  }

  static String buildDataSource(
      DeviceEndpointsBehaviourContract entity, Context context, Source source) {
    switch (source.getKind()) {
      ...
      case broadcast -> {
        ...
        String variableID =
            context.var().createVariable(GROUP_BROADCAST, id, name, VariableType.Any, null).getId();
      ...
      case property -> {
        ...
        if (endpoint == null) {
          throw new IllegalArgumentException(
              "Unable to find device endpoint: " + source.getValue() + " for device: " + entity);
        }
        return TemplateWidgetBuilder.getSource(context, endpoint);
      }
      default ->
          throw new IllegalArgumentException(
              "Unable to find handler for type: " + source.getKind());
    }
  }

  static void buildCommon(
      WidgetDefinition wd, WidgetRequest widgetRequest, WidgetBaseBuilder builder) {
  ...
  static void buildCommon(
      WidgetDefinition wd,
      WidgetRequest widgetRequest,
      WidgetBaseBuilder builder,
      Integer defaultZIndex) {
    ...
    if (margin != null && builder instanceof HasMargin<?> marginBuilder) {
      marginBuilder.setMargin(
          margin.getTop(), margin.getRight(), margin.getBottom(), margin.getLeft());
    }
  }

  static void buildBackground(
      ColorPicker background, WidgetRequest widgetRequest, WidgetBaseBuilder builder) {
    if (background == null) {
      return;
    }
    builder.setBackground(
        ...
        (Consumer<ThresholdBuilder>)
            thresholdBuilder ->
                buildThreshold(widgetRequest, background.getThresholds(), thresholdBuilder),
        (Consumer<PulseBuilder>)
            pulseBuilder ->
                buildPulseThreshold(widgetRequest, background.getPulses(), pulseBuilder));
  }

  static void buildThreshold(
      WidgetRequest widgetRequest, List<Threshold> thresholds, ThresholdBuilder thresholdBuilder) {
    if (thresholds != null) {
      for (Threshold threshold : thresholds) {
        thresholdBuilder.setThreshold(
            ...
            threshold.getOp(),
            buildDataSource(
                widgetRequest.entity(), widgetRequest.context(), threshold.getSource()));
      }
    }
  }

  static void buildPulseThreshold(
      WidgetRequest widgetRequest, List<Pulse> pulses, PulseBuilder pulseThresholdBuilder) {
    if (pulses != null) {
