...
public class ToggleTemplateWidget implements TemplateWidgetBuilder {
  ...
  public void buildMainWidget(MainWidgetRequest request) {
    ...
    widgetRequest
        ...
        .widget()
        .createToggleWidget(
            "tgl_" + entity.getEntityID(),
            builder -> {
              TemplateWidgetBuilder.buildCommon(wd, widgetRequest, builder);
              builder
                  .setDisplayType(wd.getOptions().getToggleType())
                  .setLayout(
                      UIFieldLayout.LayoutBuilder.builder(50, 50)
                          .addRow(
              ...
              for (DeviceEndpoint endpoint : includeEndpoints) {
                ...
                builder.addSeries(
                    getName(entity, deviceEndpoint),
                    seriesBuilder -> {
                      TemplateWidgetBuilder.buildIconAndColor(
                          endpoint, seriesBuilder, wbEndpoint, widgetRequest);
                      TemplateWidgetBuilder.setValueDataSource(
                              seriesBuilder, widgetRequest.context(), deviceEndpoint)
