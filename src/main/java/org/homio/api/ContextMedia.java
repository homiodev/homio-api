...
public interface ContextMedia {
  ...
  interface FFMPEG {
    ...
    @SneakyThrows
    static <T> T execute(
        @Nullable FFMPEG ffmpeg, @NotNull ThrowingFunction<FFMPEG, T, Exception> handler) {
      if (ffmpeg != null) {
    ...
    /**
     * @return if ffmpeg was started. return true even if thread is dead and getIsAlive() return
     *     false
     */
    boolean isRunning();
    ...
    /**
     * @return true if process was alive and fired stop command, false if process wasn't alive
     *     already
     */
    ...
    /**
     * @return true if process was running, alive and fired stop command, false if process wasn't
     *     alive already
     */
    ...
    FFMPEG addDestroyListener(
        @NotNull String key, @NotNull ThrowingRunnable<Exception> destroyListener);
  }
