...
public abstract class MenuBlock {
  ...
  public static class ServerMenuBlock extends MenuBlock {
    ...
    ServerMenuBlock(
      ...
      this.items =
          new MenuBlockFunction(url, keyName, valueName, new String[] {firstKey, firstValue});
    }
  ...
  public static class StaticMenuBlock<T> extends MenuBlock {
    ...
    @Setter private @Nullable Object defaultValue;
    ...
    public <T extends Enum, S extends Enum> void subMenu(T key, Class<S> subMenu) {
      ...
      this.subMenu.put(
          key.name(),
          Stream.of(subMenu.getEnumConstants()).map(Enum::name).collect(Collectors.toList()));
    }
