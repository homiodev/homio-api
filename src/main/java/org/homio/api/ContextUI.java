...
public interface ContextUI {
   ...
   */
  void updateItem(
      @NotNull BaseEntityIdentifier baseEntity,
  ...
  interface NotificationBlockBuilder {
    ...
    @NotNull
    NotificationBlockBuilder addFlexAction(
        @NotNull String key, @NotNull Consumer<UIFlexLayoutBuilder> builder);
    ...
    default @NotNull NotificationBlockBuilder setDevices(
        @Nullable Collection<? extends DeviceBaseEntity> devices) {
      if (devices != null) {
        addInfo(
            "sum",
            new Icon("fas fa-mountain-city", "#CDDC39"),
            "TITLE.DEVICES_STAT",
            Map.of(
                "ONLINE",
                devices.stream().filter(d -> d.getStatus().isOnline()).count(),
                "TOTAL",
                devices.size()));
        if (devices.isEmpty()) {
          return this;
        ...
        contextMenuActionBuilder(
            contextAction -> {
              for (DeviceBaseEntity device : devices) {
                String name =
                    device instanceof DeviceEndpointsBehaviourContract
    ...
    /**
     * Specify custom border color. Default takes color from Status if present. If border and status
     * not specified than fetch all rows from block and check if it has status info. If all rows has
     * ONLINE, ...
     *
     ...
     *
     * @param updateHandler - handler to execute when user press update button. Execution executes
     *     inside thread with passing progressBar object and selected 'version'
     * @param versions - list of versions to be able to select from UI select box
     * @return builder
     */
    @NotNull
    NotificationBlockBuilder setUpdatable(
        @NotNull
            ThrowingBiFunction<ProgressBar, String, ActionResponseModel, Exception> updateHandler,
        @NotNull List<OptionModel> versions);

    default @NotNull NotificationBlockBuilder setUpdatable(
        @NotNull HasFirmwareVersion firmwareEntity) {
    ...
    default @NotNull NotificationInfoLineBuilder addInfo(
        @NotNull String info, @Nullable Icon icon) {
    ...
    @NotNull
    NotificationInfoLineBuilder addInfo(
        @NotNull String key, @Nullable Icon icon, @NotNull String info);

    @NotNull
    default NotificationInfoLineBuilder addInfo(
        @NotNull String key, @Nullable Icon icon, @NotNull String info, @NotNull String param0) {
    ...
    default NotificationInfoLineBuilder addInfo(
        ...
        @Nullable Icon icon,
        @NotNull String info,
        @NotNull Map<String, Object> param0) {
      return addInfo(key, icon, Lang.getServerMessage(info, param0));
    }
  ...
  interface NotificationInfoLineBuilder {
    ...
    @NotNull
    NotificationInfoLineBuilder setRightText(
        @Nullable String text, @Nullable Icon icon, @Nullable String color);
    ...
    @NotNull
    NotificationButtonBuilder setRightButton(
        @Nullable Icon buttonIcon, @Nullable String buttonText, @Nullable UIActionHandler handler);

    @NotNull
    NotificationInfoLineBuilder setRightToggleButton(
        boolean value, @Nullable UIActionHandler handler);

    @NotNull
    NotificationInfoLineBuilder setRightSettingsButton(
        @NotNull Icon buttonIcon, @NotNull Consumer<UILayoutBuilder> assembler);

    @NotNull
    default NotificationInfoLineBuilder setRightSettingsButton(
  ...
  interface ContextUIProgress {
    ...
    /**
     * Create simple progress bar. Defaults: log to console: true Cancellable: false
     *
    ...
    default <T> T runAndGet(
      ...
      ProgressBar progressBar =
          (progress, message, error) -> update(progressKey, progress, message, cancellable);
  ...
  interface ContextUIMedia {
    ...
    void playWebAudio(
        @NotNull ContentStream audioStream, @Nullable Integer from, @Nullable Integer to);
  }

  interface ContextUIDialog {

    MirrorImageDialog topImageDialog(
    ...
    default void sendDialogRequest(
        @NotNull String key,
        @NotNull String title,
        @NotNull DialogRequestHandler actionHandler,
  ...
  interface ContextUIToastr {
    ...
    default void error(
        @NotNull String message, @NotNull Map<String, Object> messageParam, @NotNull Exception ex) {
    ...
    default void error(
        ...
        @Nullable Map<String, Object> messageParam,
    ...
    default void sendMessage(
      ...
      if (ex instanceof ServerException) {
        ...
        text =
            StringUtils.isEmpty(message) ? ex == null ? "Unknown error" : ex.getMessage() : message;
        if (text == null) {
