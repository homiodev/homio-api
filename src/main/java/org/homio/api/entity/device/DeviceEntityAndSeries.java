...
public abstract class DeviceEntityAndSeries<S extends DeviceSeriesEntity> extends DeviceBaseEntity {
    ...
    @MaxItems(10)
    @OneToMany(fetch = FetchType.EAGER, cascade = CascadeType.ALL, orphanRemoval = true, mappedBy = "deviceEntity", targetEntity = DeviceSeriesEntity.class)
    private Set<S> series;

    @Override
    protected long getChildEntityHashCode() {
        long code = super.getChildEntityHashCode();
        for (S s : series) {
