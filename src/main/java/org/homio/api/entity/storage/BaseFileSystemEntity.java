...
public interface BaseFileSystemEntity<FS extends FileSystemProvider>
    extends BaseEntityIdentifier, HasDynamicContextMenuActions, HasStatusAndMsg, HasPathAlias {
  ...
  default boolean createView(
      @NotNull List<TreeNode> sources, @NotNull String name, @NotNull Icon icon) {
  ...
  default @NotNull List<TreeConfiguration> buildFileSystemConfiguration() {
    ...
    for (Alias alias : getAliases()) {
      configurations.add(
          new TreeConfiguration(this, alias.getName(), alias.getAlias(), alias.getIcon()));
    }
    for (TreeConfiguration configuration : configurations) {
      configuration.setDynamicUpdateId(
          "tree-%s-%d".formatted(configuration.getId(), configuration.getAlias()));
    }
  ...
  default UIFieldProgress.Progress getSpace() {
    try {
      ...
      if (dbSize != null && dbSize.totalSpace > 0) {
        int usedPercentage =
            (int) ((dbSize.totalSpace - dbSize.freeSpace) * 100 / dbSize.totalSpace);
        String msg =
            humanReadableByteCountSI(dbSize.freeSpace)
                + " free of "
                + humanReadableByteCountSI(dbSize.totalSpace);
  ...
  default String getDescription() {
    ...
    return requireConfigure()
        ? Lang.getServerMessage(prefix.substring(0, prefix.length() - 1) + ".description")
        : null;
  }
