...
public interface FileSystemProvider {
  ...
  default @NotNull TreeNode copy(
      @NotNull TreeNode entry, @NotNull String targetId, @NotNull UploadOption uploadOption) {
    return copy(Collections.singletonList(entry), targetId, uploadOption);
  }

  // method has to search for files/folders in thread
  default @Nullable SearchThread search(
      @NotNull SearchParameters searchParameters, @NotNull SearchCallback searchCallback) {
    return () -> {
      // just ignore
    };
  }
