...
public class TreeNode implements Comparable<TreeNode> {
  ...
  @JsonIgnore private @Nullable TreeNode parent;

  @JsonIgnore private @Nullable Map<String, TreeNode> childrenMap;

  @JsonIgnore private @Nullable InputStream inputStream;

  @JsonIgnore private @Nullable FileSystemProvider fileSystem;
  ...
  public TreeNode(
      ...
      @Nullable String id,
      @Nullable Long size,
      @Nullable Long lastModifiedTime,
      @Nullable FileSystemProvider fileSystem,
  ...
  public static TreeNode of(@NotNull MultipartFile file) {
    ...
    return new TreeNode(
            false, false, name, name, file.getSize(), null, null, file.getContentType(), -1)
        .setInputStream(file.getInputStream());
  }

  public static TreeNode of(@NotNull String name, byte[] content) {
    return new TreeNode(false, false, name, name, (long) content.length, null, null, null, -1)
        .setInputStream(new ByteArrayInputStream(content));
  }

  @SneakyThrows
  public static TreeNode of(
      @Nullable String id, @NotNull Path path, @NotNull FileSystemProvider fileSystem) {
    return new TreeNode(
        false,
        false,
  ...
  public static @NotNull Pair<TreeNode, TreeNode> buildTree(@NotNull Path treePath) {
    ...
    for (Path path : treePath) {
      ...
      cursor =
          cursor.addChild(
              new TreeNode(true, false, name, name, null, null, null, null, path.hashCode()));
    }
  ...
  public void refreshOnUI(Context context) {
    if (fileSystem != null) {
      context
          .ui()
          .sendDynamicUpdate(
              "tree-%s-%d".formatted(fileSystem.getFileSystemId(), fileSystem.getFileSystemAlias()),
              this);
    }
  }
  ...
  public @Nullable TreeNode clone(boolean includeRelations) {
    return new TreeNode(
        ...
        this.uuid,
        attributes,
        includeRelations ? this.parent : null,
        includeRelations ? childrenMap : null,
        inputStream,
        fileSystem);
  }

  public @NotNull TreeNode addChild(
      @NotNull String id, boolean appendParentId, @NotNull Supplier<TreeNode> supplier) {
    if (this.childrenMap == null) {
    ...
    for (Path child : path) {
      ...
      cursor =
          children == null
              ? null
              : children.stream()
                  .filter(c -> c.name.equals(child.getFileName().toString()))
                  .findAny()
                  .orElse(null);
      if (cursor == null) {
        return null;
  ...
  public @Nullable TreeNode addChildIfNotFound(
      @NotNull String id, @NotNull Supplier<TreeNode> childSupplier, boolean appendParentId) {
    TreeNode treeNode =
        appendParentId && this.id != null ? this.getChild(this.id + "/" + id) : this.getChild(id);
    if (treeNode == null) {
