...
public class TarPath implements Path {
  ...
  public URI toUri() {
    try {
      return new URI(
          "tar", tfs.getTarFile().toUri() + "!" + new String(toAbsolutePath().path), null);
    } catch (Exception ex) {
  ...
  public Path relativize(Path other) {
    ...
    if (
    /* this.getFileSystem() != o.getFileSystem() || */
  ...
  public boolean startsWith(Path other) {
    ...
    olast--;
    return o.path.length == path.length || o.path[olast] == '/' || path[olast + 1] == '/';
  }
  ...
  boolean isSameFile(Path other) throws IOException {
    ...
    return Arrays.equals(getResolvedPath(), ((TarPath) other).getResolvedPath());
  }
  ...
  void move(TarPath target, CopyOption... options) throws IOException {
    if (Files.isSameFile(tfs.getTarFile(), target.tfs.getTarFile())) {
      tfs.copyFile(true, getResolvedPath(), target.getResolvedPath(), options);
    } else {
  ...
  void copy(TarPath target, CopyOption... options) throws IOException {
    if (Files.isSameFile(tfs.getTarFile(), target.tfs.getTarFile())) {
      tfs.copyFile(false, getResolvedPath(), target.getResolvedPath(), options);
    } else {
      copyToTarget(target, options);
    }
  }
  ...
  private void copyToTarget(TarPath target, CopyOption... options) throws IOException {
    ...
    if (copyAttrs) {
      BasicFileAttributeView view = TarFileAttributeView.get(target, BasicFileAttributeView.class);
      try {
        view.setTimes(tfas.lastModifiedTime(), tfas.lastAccessTime(), tfas.creationTime());
      } catch (IOException x) {
        // rollback?
        try {
          target.delete();
