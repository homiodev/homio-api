...
public abstract class BaseCachedFileSystemProvider<
        Entity extends BaseFileSystemEntity,
        FSFile extends FsFileEntity<FSFile>,
        Service extends BaseFSService<FSFile>>
  ...
  @Getter protected @NotNull Entity entity;
  ...
  public BaseCachedFileSystemProvider(@NotNull Entity entity, @NotNull Context context) {
    ...
    this.fileCache =
        CacheBuilder.newBuilder()
            .expireAfterWrite(entity.getFileSystemCacheTimeout(), TimeUnit.MINUTES)
            .build(
                new CacheLoader<>() {
  ...
  @Override
  @SneakyThrows
  public @Nullable TreeNode create(
      @NotNull String parentId, @NotNull String name, boolean isDir, UploadOption uploadOption) {
  ...
  @SneakyThrows
  @Override
  public @Nullable TreeNode rename(
      @NotNull String id, @NotNull String newName, UploadOption uploadOption) {
  ...
  public @NotNull TreeNode copy(
      @NotNull Collection<TreeNode> entries,
  ...
  public @Nullable Set<TreeNode> loadTreeUpToChild(@Nullable String parent, @NotNull String id) {
    ...
    List<String> items =
        StreamSupport.stream(Paths.get(file.getAbsolutePathWithoutRoot()).spliterator(), false)
            .map(Path::toString)
            .toList();
    for (int i = 0; i < items.size() - 1; i++) {
      ...
      TreeNode foundedObject =
          currentChildren.stream()
              .filter(c -> c.getName().equals(pathItem))
              .findAny()
  ...
  @SneakyThrows
  private @NotNull TreeNode buildRoot(
      @NotNull Collection<FSFile> result, boolean handleAttributes) {
  ...
  private @Nullable FSFile getFSFile(String id) {
    ...
    try {
      ...
      String parentId =
          StringUtils.defaultIfEmpty(fixPath(Paths.get(fileId).getParent()), getNullParentId());
      fsFile =
          fileCache.get(parentId).stream()
              .filter(
                  c -> {
