...
public interface UILayoutBuilder extends UIEntityBuilder {
  ...
  default UIFlexLayoutBuilder addFlex(
      @NotNull String name, @NotNull Consumer<UIFlexLayoutBuilder> flexConsumer) {
  ...
  default UIInfoItemBuilder addInfo(
      @NotNull String name, @NotNull UIInfoItemBuilder.InfoType infoType) {
    return addInfo(name, infoType, getNextOrder());
  }

  UIInfoItemBuilder addInfo(
      @NotNull String name, @NotNull UIInfoItemBuilder.InfoType infoType, int order);
  ...
  default UITextInputItemBuilder addTextInput(
      @NotNull String name, @Nullable String defaultValue, boolean required) {
    return addInput(name, defaultValue, UITextInputItemBuilder.InputType.Text)
        .setRequired(required);
  }

  default UISelectBoxItemBuilder addSelectBox(
      @NotNull String name, @Nullable UIActionHandler action) {
  ...
  UISelectBoxItemBuilder addSelectBox(
      @NotNull String name, @Nullable UIActionHandler action, int order);

  default UICheckboxItemBuilder addCheckbox(
      @NotNull String name, boolean value, @Nullable UIActionHandler action) {
    return addCheckbox(name, value, action, getNextOrder());
  }

  UICheckboxItemBuilder addCheckbox(
      @NotNull String name, boolean value, @Nullable UIActionHandler action, int order);

  default UIMultiButtonItemBuilder addMultiButton(
      @NotNull String name, @Nullable UIActionHandler action) {
    return addMultiButton(name, action, getNextOrder());
  }

  UIMultiButtonItemBuilder addMultiButton(
      @NotNull String name, @Nullable UIActionHandler action, int order);

  default UISliderItemBuilder addSlider(
      @NotNull String name, float value, float min, float max, @Nullable UIActionHandler action) {
    return addSlider(
        name, value, min, max, action, UISliderItemBuilder.SliderType.Regular, getNextOrder());
  }

  default UISliderItemBuilder addNumberInput(
      @NotNull String name, Float value, Float min, Float max, UIActionHandler action) {
    return addSlider(
            name, value, min, max, action, UISliderItemBuilder.SliderType.Input, getNextOrder())
  ...
  DialogEntity<UIDialogLayoutBuilder> addOpenDialogActionButton(
      @NotNull String name, @Nullable Icon icon, @Nullable Integer width, int order);
